package com.vandergriff.validation;

public class UnexpectedResultException extends Exception {

	private static final long serialVersionUID = 2144583525524470864L;

	/**
	 * 
	 */
	public UnexpectedResultException() {
		super();
	}

	/**
	 * @param message
	 * @param cause
	 * @param enableSuppression
	 * @param writableStackTrace
	 */
	public UnexpectedResultException(String message, Throwable cause, boolean enableSuppression,
			boolean writableStackTrace) {
		super(message, cause, enableSuppression, writableStackTrace);
	}

	/**
	 * @param message
	 * @param cause
	 */
	public UnexpectedResultException(String message, Throwable cause) {
		super(message, cause);
	}

	/**
	 * @param message
	 */
	public UnexpectedResultException(String message) {
		super(message);
	}

	/**
	 * @param cause
	 */
	public UnexpectedResultException(Throwable cause) {
		super(cause);
	}

}
